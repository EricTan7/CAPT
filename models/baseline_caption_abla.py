...
# ==================================== ablation ========================================================
class PromptLearner_caption_abl_wo_textsup(nn.Module):
...
class CustomCLIP_caption_abl_wo_textsup(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        super().__init__()
        device = 'cuda'
        self.prompt_learner = PromptLearner_caption_abl_wo_textsup(cfg, classnames, clip_model)
...
class Baseline_caption_abl_wo_textsup(BaseModel):
    def __init__(self, cfg, classnames=None):
        ...
        self.logger.info("Building Baseline_caption_abl_wo_caption")
        self.model = CustomCLIP_caption_abl_wo_textsup(cfg, classnames, clip_model)
...
class PromptLearner_caption_abl_wo_inssup(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        super().__init__()
        n_cls = len(classnames)
        n_ctx = cfg.TRAINER.BASELINE.N_CTX
...
class CustomCLIP_caption_abl_wo_inssup(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        super().__init__()
        device = 'cuda'
        self.prompt_learner = PromptLearner_caption_abl_wo_inssup(cfg, classnames, clip_model)
        self.image_encoder = clip_model.visual
        self.text_encoder = TextEncoder(clip_model)
        self.logit_scale = torch.tensor(4.60517)
        self.dtype = clip_model.dtype
...
class Baseline_caption_abl_wo_inssup(BaseModel):
    def __init__(self, cfg, classnames=None):
        super().__init__()
        self.logger = logging.getLogger(cfg.TRAINER.NAME)
        self.check_cfg(cfg)
        ...
        self.logger.info("Building Baseline_caption_abl_wo_captionsup")
        self.model = CustomCLIP_caption_abl_wo_inssup(cfg, classnames, clip_model)
...
class PromptLearner_caption_abl_wo_ctgsup(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        super().__init__()
        n_cls = len(classnames)
        n_ctx = cfg.TRAINER.BASELINE.N_CTX
...
class CustomCLIP_caption_abl_wo_ctgsup(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        super().__init__()
        device = 'cuda'
        self.prompt_learner = PromptLearner_caption_abl_wo_ctgsup(cfg, classnames, clip_model)
        self.image_encoder = clip_model.visual
        self.text_encoder = TextEncoder(clip_model)
        self.logit_scale = torch.tensor(4.60517)
        self.dtype = clip_model.dtype
...
class Baseline_caption_abl_wo_ctgsup(BaseModel):
    def __init__(self, cfg, classnames=None):
        super().__init__()
        self.logger = logging.getLogger(cfg.TRAINER.NAME)
        self.check_cfg(cfg)
        ...
        self.logger.info("Building Baseline_caption_abl_wo_captionsup")
        self.model = CustomCLIP_caption_abl_wo_ctgsup(cfg, classnames, clip_model)
