...
class PromptLearner_shembed_zsinit_2xcattn(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        num_query_token = 32
        hidden_act = 'gelu'
        self.bonder = CrossAttnBlock_nx(ctx_dim, num_heads=8, depth=2)
        self.query = nn.Parameter(torch.zeros(1, num_query_token, ctx_dim))
        self.query.data.normal_(mean=0.0, std=0.02)
        print(f"Number of queries: {num_query_token}")
        # self.token_embedding = nn.Embedding(vocab_size, transformer_width)
...
class PromptLearner_shembed_zsinit_2xcattn_pe(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        num_query_token = 32
        hidden_act = 'gelu'
        self.bonder = CrossAttnBlock_nx_pe(ctx_dim, input_size=(num_query_token, ctx_dim), num_heads=8, depth=2)
        self.query = nn.Parameter(torch.zeros(1, num_query_token, ctx_dim))
        self.query.data.normal_(mean=0.0, std=0.02)
        print(f"Number of queries: {num_query_token}")
        # self.token_embedding = nn.Embedding(vocab_size, transformer_width)
...
# ===================================== zeroshot init =======================
class PromptLearner_shembed_zsinit_lscale(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        super().__init__()
        n_cls = len(classnames)
        n_ctx = cfg.TRAINER.BASELINE.N_CTX
...
class CustomCLIP_shembed_zsinit_lscale(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        super().__init__()
        self.prompt_learner = PromptLearner_shembed_zsinit_lscale(cfg, classnames, clip_model)
        self.tokenized_prompts = self.prompt_learner.tokenized_query
        self.image_encoder = clip_model.visual
        self.text_encoder = TextEncoder(clip_model)
...
class Baseline_cattn_vocabloss_shembed_zsinit_lscale(BaseModel):
    def __init__(self, cfg, classnames=None):
        super().__init__()
        self.logger = logging.getLogger(cfg.TRAINER.NAME)
        self.check_cfg(cfg)
        ...
        self.logger.info("Building Baseline_cattn_vocabloss_shembed_zsinit_lscale")
        self.model = CustomCLIP_shembed_zsinit_lscale(cfg, classnames, clip_model)
...
# ===================================== zeroshot init =======================
class PromptLearner_shembed_zsinit_mul_lscale_wiseft(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        super().__init__()
        n_cls = len(classnames)
        n_ctx = cfg.TRAINER.BASELINE.N_CTX
...
# note: to align with new repo, use all tokens
class CustomCLIP_shembed_zsinit_mul_lscale_wiseft(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        super().__init__()
        device = 'cuda'
        self.prompt_learner = PromptLearner_shembed_zsinit_mul_lscale_wiseft(cfg, classnames, clip_model)
        self.tokenized_prompts = self.prompt_learner.tokenized_query
        self.image_encoder = clip_model.visual
        self.text_encoder = TextEncoder(clip_model)
        self.logit_scale = torch.tensor(4.60517)
...
class Baseline_cattn_vocabloss_shembed_zsinit_mul_lscale_wiseft(BaseModel):
    def __init__(self, cfg, classnames=None):
        super().__init__()
        self.logger = logging.getLogger(cfg.TRAINER.NAME)
        self.check_cfg(cfg)
        ...
        self.logger.info("Building Baseline_cattn_vocabloss_shembed_zsinit_mul_lscale_wiseft")
        self.model = CustomCLIP_shembed_zsinit_mul_lscale_wiseft(cfg, classnames, clip_model)
...
# ===================================== zeroshot init =======================
class PromptLearner_shembed_zsinit_lscale_wiseft_nxcattn(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        super().__init__()
        n_cls = len(classnames)
        n_ctx = cfg.TRAINER.BASELINE.N_CTX
        ...
        self.bonder = CrossAttnBlock_nx_projkv(ctx_dim, num_heads=8, depth=depth)
        self.query = nn.Parameter(torch.zeros(1, num_query_token, ctx_dim))
        self.query.data.normal_(mean=0.0, std=0.02)
        print(f"Number of queries: {num_query_token}")
...
class CustomCLIP_shembed_zsinit_lscale_wiseft_nxcattn(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        super().__init__()
        device = 'cuda'
        self.prompt_learner = PromptLearner_shembed_zsinit_lscale_wiseft_nxcattn(cfg, classnames, clip_model)
        self.tokenized_prompts = self.prompt_learner.tokenized_query
        self.image_encoder = clip_model.visual
        self.text_encoder = TextEncoder(clip_model)
        self.logit_scale = torch.tensor(4.60517)
...
class Baseline_cattn_vocabloss_shembed_zsinit_lscale_wiseft_nxcattn(BaseModel):
    def __init__(self, cfg, classnames=None):
        super().__init__()
        self.logger = logging.getLogger(cfg.TRAINER.NAME)
        self.check_cfg(cfg)
        ...
        self.logger.info("Building Baseline_cattn_vocabloss_shembed_zsinit_lscale_wiseft_nxcattn")
        self.model = CustomCLIP_shembed_zsinit_lscale_wiseft_nxcattn(cfg, classnames, clip_model)
...
# ===================================== zeroshot init =======================
class PromptLearner_shembed_zsinit_lscale_lnable_wiseft_nxcattn(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        super().__init__()
        n_cls = len(classnames)
        n_ctx = cfg.TRAINER.BASELINE.N_CTX
...
class CustomCLIP_shembed_zsinit_lscale_lnable_wiseft_nxcattn(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        super().__init__()
        device = 'cuda'
        self.prompt_learner = PromptLearner_shembed_zsinit_lscale_lnable_wiseft_nxcattn(cfg, classnames, clip_model)
        self.tokenized_prompts = self.prompt_learner.tokenized_query
        self.image_encoder = clip_model.visual
        self.text_encoder = TextEncoder(clip_model)
        self.dtype = clip_model.dtype
        ...
        self.cls_head = ClsHead_cat_lscale_lnable(classnames, clip_model)
        self.wiseft_head = ClsHead_cat_lscale_lnable(classnames, clip_model)
...
class Baseline_cattn_vocabloss_shembed_zsinit_lscale_lnable_wiseft_nxcattn(BaseModel):
    def __init__(self, cfg, classnames=None):
        super().__init__()
        self.logger = logging.getLogger(cfg.TRAINER.NAME)
        self.check_cfg(cfg)
        ...
        self.logger.info("Building Baseline_cattn_vocabloss_shembed_zsinit_lscale_lnable_wiseft_nxcattn")
        self.model = CustomCLIP_shembed_zsinit_lscale_lnable_wiseft_nxcattn(cfg, classnames, clip_model)
...
# ===================================== zeroshot init =======================
class PromptLearner_shembed_zsinit_lscale_fixed(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        super().__init__()
        n_cls = len(classnames)
        n_ctx = cfg.TRAINER.BASELINE.N_CTX
...
# note: to align with new repo, use all tokens
class CustomCLIP_shembed_zsinit_lscale_fixed(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        super().__init__()
        device = 'cuda'
        self.prompt_learner = PromptLearner_shembed_zsinit_lscale_fixed(cfg, classnames, clip_model)
        self.tokenized_prompts = self.prompt_learner.tokenized_query
        self.image_encoder = clip_model.visual
        self.text_encoder = TextEncoder(clip_model)
        self.logit_scale = torch.tensor(4.60517)
...
class Baseline_cattn_vocabloss_shembed_zsinit_lscale_fixed(BaseModel):
    def __init__(self, cfg, classnames=None):
        super().__init__()
        self.logger = logging.getLogger(cfg.TRAINER.NAME)
        self.check_cfg(cfg)
        ...
        self.logger.info("Building Baseline_cattn_vocabloss_shembed_zsinit_lscale_fixed")
        self.model = CustomCLIP_shembed_zsinit_lscale_fixed(cfg, classnames, clip_model)
...
# ===================================== zeroshot init =======================
class PromptLearner_shembed_zsinit_lscale_wiseft_wotextloss(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        super().__init__()
        n_cls = len(classnames)
        n_ctx = cfg.TRAINER.BASELINE.N_CTX
...
# note: to align with new repo, use all tokens
class CustomCLIP_shembed_zsinit_lscale_wiseft_wotextloss(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        super().__init__()
        device = 'cuda'
        self.prompt_learner = PromptLearner_shembed_zsinit_lscale_wiseft_wotextloss(cfg, classnames, clip_model)
        self.tokenized_prompts = self.prompt_learner.tokenized_query
        self.image_encoder = clip_model.visual
        self.text_encoder = TextEncoder(clip_model)
        self.logit_scale = torch.tensor(4.60517)
...
class Baseline_cattn_vocabloss_shembed_zsinit_lscale_wiseft_wotextloss(BaseModel):
    def __init__(self, cfg, classnames=None):
        super().__init__()
        self.logger = logging.getLogger(cfg.TRAINER.NAME)
        self.check_cfg(cfg)
        ...
        self.logger.info("Building Baseline_cattn_vocabloss_shembed_zsinit_lscale_wiseft_wotextloss")
        self.model = CustomCLIP_shembed_zsinit_lscale_wiseft_wotextloss(cfg, classnames, clip_model)
