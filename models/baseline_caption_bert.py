...
class PromptLearner_caption_multi_stream_bert(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.text_encoder = BertModel.from_pretrained(cfg.MODEL.TEXT.ENCODER)
        word_embed_layer = self.text_encoder.get_input_embeddings()
...
class CustomCLIP_caption_multi_stream_bert(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        super().__init__()
        device = 'cuda'
        self.prompt_learner = PromptLearner_caption_multi_stream_bert(cfg, classnames, clip_model)
    ...
    def forward(self, image, target=None, caption=None):  # image: [B,3,224,224]  label:[B]
        ...
        text_features_category = self.prompt_learner.text_encoder(inputs_embeds=prompts_category).pooler_output  # [B,768]
        text_features_instance = self.prompt_learner.text_encoder(inputs_embeds=prompts_instance).pooler_output
...
class Baseline_caption_wiseft_multi_stream_bert(BaseModel):
    def __init__(self, cfg, classnames=None):
        ...
        self.logger.info("Building Baseline_caption_wiseft_multi_stream_bert")
        self.model = CustomCLIP_caption_multi_stream_bert(cfg, classnames, clip_model)
