...
class Baseline_caption_wiseft_se_pre_all(BaseModel):
    def __init__(self, cfg, classnames=None):
        ...
        self.optim = build_optimizer([self.model.prompt_learner, self.model.cls_head, self.model.se_text, self.model.se_img], cfg.OPTIM)
...
class CustomCLIP_caption_se_cross(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        super().__init__()
        device = 'cuda'
        self.prompt_learner = PromptLearner_caption(cfg, classnames, clip_model)
        ...
        self.se = cross_se_layer(clip_model.visual.output_dim, reduction=16)
    ...
    def forward(self, image, target=None, caption=None):  # image: [B,3,224,224]  label:[B]
        ...
        image_cls, text_features = self.se(image_cls, text_features)
        fused_fea = torch.cat([image_cls, text_features], dim=1)
        logits = self.cls_head(fused_fea)
        if not self.prompt_learner.training:
            logits_wiseft = self.wiseft_head(fused_fea)
...
class Baseline_caption_wiseft_se_cross(BaseModel):
    def __init__(self, cfg, classnames=None):
        super().__init__()
        self.logger = logging.getLogger(cfg.TRAINER.NAME)
        self.check_cfg(cfg)
        ...
        self.logger.info("Building Baseline_caption_wiseft_se_cross")
        self.model = CustomCLIP_caption_se_cross(cfg, classnames, clip_model)
