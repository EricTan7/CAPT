...
val_shot_list = {2: 2, 4: 4, 8: 4, 16: 4}
...
for num_shot in [2, 4, 8, 16]:
    ...
    for seed in range(1, args.num_run + 1):
        ...
        for c_weight in search_list:
            clf = LogisticRegression(solver="lbfgs", max_iter=1000, penalty="l2", C=c_weight, n_jobs=2).fit(fewshot_train_feature, fewshot_train_label)
        ...
        def binary_search(c_left, c_right, seed, step, test_acc_step_list):
            clf_left = LogisticRegression(solver="lbfgs", max_iter=1000, penalty="l2", C=c_left, n_jobs=2).fit(fewshot_train_feature, fewshot_train_label)
            ...
            clf_right = LogisticRegression(solver="lbfgs", max_iter=1000, penalty="l2", C=c_right, n_jobs=2).fit(fewshot_train_feature, fewshot_train_label)
            ...
            # ) as writer:
            with open(
                    "/mnt/sdc/tanhao/Baseline_v1/lpclip/report/clip_feat_s{}r{}_details.txt".format(args.num_step, args.num_run),
                    "a+",
            ) as writer:
    ...
    # ) as writer:
    with open(
            "/mnt/sdc/tanhao/Baseline_v1/lpclip/report/clip_feat_s{}r{}.txt".format(args.num_step, args.num_run),
            "a+",
    ) as writer:
